...
2. Download the dataset from Kaggle and place it at the path specified above
3. Open `visual-intelligence.ipynb` in Jupyter or Kaggle
4. Run all cells in order — training, evaluation, visualisation, and XAI are all sequential

> The CNN best model is saved automatically during K-Fold training. If running ScatNet XAI, ensure the ScatNet model variable (`model`) is still in memory from the training cell, as the ScatNet weights are not saved to disk.

---

## References

1. Borkowski, A. A. et al. (2019). *Lung and Colon Cancer Histopathological Image Dataset (LC25000)*. arXiv:1912.12142.
2. Kaggle dataset: https://www.kaggle.com/datasets/rm1000/lung-cancer-histopathological-images
...
5. Fisher, A., Rudin, C., & Dominici, F. (2019). *All Models are Wrong, but Many are Useful*. JMLR, 20(177), 1–81.
6. Molnar, C. (2022). *Interpretable Machine Learning* (2nd ed.). https://christophm.github.io/interpretable-ml-book/
